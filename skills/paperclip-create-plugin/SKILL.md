...
description: >
  Create new Team Dashboard plugins with the current alpha SDK/runtime. Use when
...
# Create a Team Dashboard Plugin

Use this skill when the task is to create, scaffold, or document a Team Dashboard plugin.
...
For a plugin that lives outside the Team Dashboard repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperclip-sdk/`:
...
- is installed into Team Dashboard from an absolute local path during development
