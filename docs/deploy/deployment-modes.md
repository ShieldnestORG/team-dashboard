...
Team Dashboard supports two runtime modes with different security profiles.
...
When migrating from `local_trusted` to `authenticated`, Team Dashboard emits a one-time claim URL at startup:
