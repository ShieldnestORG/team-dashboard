...
| **Causal Events (PyRapide-style observability)** | | | |
| `CAUSAL_EVENTS_ENABLED` | Optional | VPS | Master kill switch for `recordEvent()`. Default `true`. Set to `false` if the `caused_by` / `event_kind` column writes misbehave — `recordEvent` becomes a no-op and observed code keeps running. See [Causal Events](../operations/causal-events.md). |
| `CAUSAL_CONSTRAINTS_ENABLED` | Optional | VPS | Kill switch for the every-5-min `causal-constraints:check` cron + default constraint seeding. Default `true`. Set to `false` to stop the cron registering at boot (e.g. if the violation query is generating spurious `causal.constraint.violated` events). Does NOT disable `recordEvent` itself — use `CAUSAL_EVENTS_ENABLED` for the master switch. |
