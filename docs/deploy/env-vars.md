...
| `VOICE_SNIPPETS_DAILY_LIMIT` | Optional | VPS | Per-user daily cap on PAID voice generations (cache misses) for `POST /api/voice-snippets`; over the cap → plain-English 429. Default 200. Cached lines never count. |
| **Platform Publishing** | | | |
...
| `SYNTHETIC_MONITOR_ENABLED` | Optional | VPS | `true` to enable the synthetic uptime monitor (`server/src/services/synthetic-monitor.ts`), now wired to the `monitor:synthetic-canary` cron (every 15 min, owner `nova`) — Playwright-rendered canary checks that capture JS console errors, uncaught page errors, broken images, and load time; alerts after 2 consecutive failures. Requires Playwright browsers installed in the runtime (`pnpm exec playwright install chromium`). Default `false` (registers no cron when unset). |
| `SYNTHETIC_MONITOR_URLS` | Optional | VPS | Comma-separated URLs for the synthetic canary to check when `SYNTHETIC_MONITOR_ENABLED=true`. Defaults to the key public surfaces (coherencedaddy.com + creditscore / affiliates / directory). |
| `CRON_CIRCUIT_BREAKER_THRESHOLD` | Optional | VPS | Consecutive-failure count after which the cron registry trips a job's circuit breaker (fires one `cron_stale` alert + auto-disables the job). Default `5`, floored at `1`. In-memory per process — resets on restart and on any successful run. |
| `CRON_CIRCUIT_BREAKER_ENABLED` | Optional | VPS | When `false`, the circuit breaker alerts on a crash-looping cron but does NOT auto-disable it (alert-only). Default `true` (auto-disable on). |
...
| `CREDITSCORE_CALLBACK_KEY` | Optional | VPS + coherencedaddy | HMAC shared secret for email callback from team-dashboard → storefront. If unset, emails are skipped. **University Sessions go-live (2026-06-30): REUSE the existing prod value — copy the current VPS value into the Vercel landing project; do NOT rotate casually.** It is a symmetric HMAC (td signer ↔ landing verifier) that ALSO signs unsubscribe + member-login (magic-link) tokens, so rotating invalidates every outstanding unsubscribe/login link and risks callback 400s if the two sides drift. |
| `CREDITSCORE_EMAIL_CALLBACK_URL` | Optional | VPS | Storefront endpoint that renders + sends via Brevo (default: `https://freetools.coherencedaddy.com/api/email/creditscore`) |
| `EMAIL_EVENTS_KEY` | Required for email analytics | VPS + Vercel (storefront) | HMAC shared secret for the INBOUND leg of email analytics: the storefront forwards Brevo open/click/bounce events to `POST /api/university/email-events`, signed `X-Email-Events-Signature: v1=<hex>` over the raw body. Fail-closed: the route 500s when unset and 401s on a bad signature. Pairs with coherencedaddy-landing branch `feat/email-event-analytics`. |
| `WATCHTOWER_STRIPE_PRICE_ID` | Optional | VPS | Stripe price ID for Watchtower ($49/mo). Fallback when `lookup_key=watchtower_monthly` resolution fails. |
...
| `WATCHTOWER_EMAIL_CALLBACK_URL` | Optional | VPS | Storefront endpoint that handles both `answer_check_report` and `watchtower_weekly_digest` Brevo dispatch (default: `https://freetools.coherencedaddy.com/api/email/watchtower` — 301-redirects to coherencedaddy.com). |
| **Payments — Coherent Ones University (separate Stripe account: Starwise Ventures)** | | | |
| `UNIVERSITY_STRIPE_SECRET_KEY` | Required for University in prod | VPS | Secret key for the **Starwise Ventures** Stripe account that Coherent Ones University bills on. University is the ONE product NOT on the Coherence Daddy account. Used by the University price lookup + checkout and the billing-portal session for University members. **Falls back to `STRIPE_SECRET_KEY` when unset** (single-account local/dev/test); in prod it must be the Starwise key or University checkout hits the wrong account. The global `STRIPE_SECRET_KEY` stays the CD key for every other product. See `docs/deploy/stripe-runbook.md` §Account inventory. |
| `UNIVERSITY_STRIPE_PRICE_ID` | Optional | VPS | Bare Starwise price ID for the FOUNDING monthly price ($50/mo). Fallback only — University resolves by `lookup_key=university_monthly` first. Leave unset to keep the lookup_key path canonical. |
| `UNIVERSITY_ANNUAL_PRICE_ID` | Optional | VPS | Bare Starwise price ID for the FOUNDING annual price ($500/yr). Fallback only — resolves by `lookup_key=university_annual` first. |
| `UNIVERSITY_STRIPE_STANDARD_PRICE_ID` | Optional | VPS | Bare Starwise price ID for the STANDARD monthly price ($79/mo) sold once the Founding-100 cap fills. Fallback only — resolves by `lookup_key=university_monthly_standard` first. If NEITHER resolves once the cap is reached, checkout **fails closed (503)** rather than underselling — create the price before seat 100 fills. See `docs/university-founding-pricing.md`. |
| `UNIVERSITY_ANNUAL_STANDARD_PRICE_ID` | Optional | VPS | Bare Starwise price ID for the STANDARD annual price ($790/yr — "two months free", owner-priced 2026-07-09, live as `price_1TrLvUAf8PjDIzDYhXFQbKdl`). Fallback only — resolves by `lookup_key=university_annual_standard` first. Leave unset to keep the lookup_key path canonical. |
| `UNIVERSITY_FOUNDING_CAP` | Optional (default 100) | VPS | The Founding-100 cap. **Load-bearing since the price switch**: checkout charges the founding rate while `COUNT(university_members) < cap`, the standard rate after. `0` disables the founding offer entirely. Set to `100` on VPS4 since 2026-06-20. |
| `STRIPE_WEBHOOK_SECRET_UNIVERSITY` | Optional | VPS | Dedicated signing secret for the University Stripe webhook, registered on the **Starwise** account. Falls back to `STRIPE_WEBHOOK_SECRET`, but the global is for the CD Intel endpoint — set this once the Starwise webhook exists. |
| `UNIVERSITY_SUCCESS_URL` | Optional | VPS | Stripe success_url base for University checkout. Default: `https://app.coherencedaddy.com/university`. |
| `UNIVERSITY_CANCEL_URL` | Optional | VPS | Stripe cancel_url base for University checkout. Default: `https://coherencedaddy.com/university`. |
| **Google Ads — University purchase conversion upload (all-or-nothing group: until EVERY required var is set, uploads are logged instead of sent — see `docs/deploy/google-ads-conversions.md`)** | | | |
| `GOOGLE_ADS_DEVELOPER_TOKEN` | Required for Ads upload | VPS | Google Ads API developer token (Ads UI → Tools → API Center on the MANAGER account). |
| `GOOGLE_ADS_OAUTH_CLIENT_ID` | Required for Ads upload | VPS | OAuth2 client id (Google Cloud console, same project the developer token was approved for). |
| `GOOGLE_ADS_OAUTH_CLIENT_SECRET` | Required for Ads upload | VPS | OAuth2 client secret. |
| `GOOGLE_ADS_OAUTH_REFRESH_TOKEN` | Required for Ads upload | VPS | Refresh token minted for a Google user with access to the Ads account (scope `https://www.googleapis.com/auth/adwords`). |
| `GOOGLE_ADS_CUSTOMER_ID` | Required for Ads upload | VPS | The Ads account id the conversion action lives in (dashes tolerated — `226-801-4496` and `2268014496` both work). |
| `GOOGLE_ADS_LOGIN_CUSTOMER_ID` | Optional | VPS | Manager (MCC) account id, only when API access is delegated through one. |
| `GOOGLE_ADS_UNIVERSITY_CONVERSION_ACTION_ID` | Required for Ads upload | VPS | Numeric id of the **University purchase** conversion action (create it as an IMPORT-type action — see the runbook). |
| `GOOGLE_ADS_API_VERSION` | Optional (default `v21`) | VPS | Google Ads REST API version segment. Bump here when Google sunsets the pinned default — no redeploy of code needed beyond the env change. |
| `UNIVERSITY_SESSION_ADMINS` | Required for Sessions admin | VPS | Comma-separated email allowlist for University **Sessions** admin actions (comma-sep, case-insensitive; read in `server/src/routes/portal.ts`, checked AFTER portal membership). Prod value set 2026-06-30 (UBIT legal read CLEARED → Sessions go-live): `nestd@pm.me,info@coherencedaddy.com`. Already present in this repo's local `.env`. **Note:** the two feature switches that turn Sessions ON — `UNIVERSITY_SESSION_UPSELL_ENABLED` and `UNIVERSITY_BRING_A_FRIEND_ENABLED` — are NOT read here; they live on the Vercel **landing** + **portal** projects and must be set to the literal string `"true"` (portal strict-compares `=== "true"`, so `"1"` works on landing but silently fails on portal; code default is OFF). |
