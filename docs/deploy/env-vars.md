...
| **Ad attribution — Coherent Ones University (server-side conversions)** | | | |
| `META_PIXEL_ID` | Optional | VPS | Meta (Facebook) Pixel ID for the University Conversions API. Fires a server-side `Purchase` on University checkout completion (`services/meta-capi.ts`). **No-ops safely when unset** (or when `META_CAPI_ACCESS_TOKEN` is unset) — the webhook never fails on a missing token. Pair with the browser Pixel using the same `event_id` (the Stripe checkout session id) for dedup. |
| `META_CAPI_ACCESS_TOKEN` | Optional | VPS | Meta Conversions API access token. Required alongside `META_PIXEL_ID` for the server-side `Purchase` to fire; either unset → CAPI no-ops. Secret — do not commit. |
| `TIKTOK_PIXEL_ID` | Optional | VPS | TikTok Pixel ID for the Events API. Fires a server-side `CompletePayment` on University checkout completion (`services/tiktok-events.ts`). **No-ops safely when unset** (or when `TIKTOK_EVENTS_TOKEN` is unset). Same `event_id` (Stripe session id) as the browser Pixel for dedup. |
| `TIKTOK_EVENTS_TOKEN` | Optional | VPS | TikTok Events API access token. Required alongside `TIKTOK_PIXEL_ID` for the server-side `CompletePayment` to fire; either unset → TikTok no-ops. Secret — do not commit. |
