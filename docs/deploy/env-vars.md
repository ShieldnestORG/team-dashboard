...
| `UNIVERSITY_ANNUAL_STANDARD_PRICE_ID` | Optional | VPS | Bare Starwise price ID for the STANDARD annual price (owner has not priced it yet). Fallback only — resolves by `lookup_key=university_annual_standard` first. Until one of the two exists, the annual plan is unavailable past the founding cap (fails closed). |
