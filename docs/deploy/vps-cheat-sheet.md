...
| Leaving a "rollback" team-dashboard container **running** on a second box | It shares the prod Neon DB, so every cron runs **twice** — and the copy runs frozen, pre-fix code. It re-poisons shared state (`system_crons.last_error`, duplicate sends) for months, making already-fixed bugs look alive. | A rollback copy must be **stopped**: `docker update --restart=no <name> && docker stop <name>`. Verify with `docker ps` on every box after any swap. |
...
2026-07-09: discovered the `.12` team-dashboard container (image built 2026-05-10, `restart: unless-stopped`) had been **running the whole two months since the swap**, double-executing all ~126 crons against prod Neon with pre-fix code and a dead VPS3 `EMBED_URL`. It kept writing `last_error` into shared `system_crons` rows, so VPS4's watchdog emailed "N cron jobs unhealthy" for months after the underlying bugs were fixed — the alerts were honest, the state was poisoned. **Net cost: a summer of bogus alert emails + at least two prior debugging sessions chasing "transient" cron failures that were this container all along.** Stopped + `restart=no` 2026-07-09; see [Alerting Policy](../operations/alerting-policy.md).
