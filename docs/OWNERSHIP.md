...
| **Owned utility-site registry** (domains we own for ad-revenue arbitrage) | **team-dashboard** (`owned_sites`, `owned_site_metrics` tables) | Sites are static HTML; hosting target was VPS3 nginx pre-2026-05-08, now re-picking (VPS3 decommissioning post-XMRig). team-dashboard aggregates GA4/AdSense metrics and exposes the portfolio at `/owned-sites`. Strategy doc: `docs/products/utility-network/README.md`. |
