...
Team Dashboard is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── paperclip/               # Core Team Dashboard skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Team Dashboard env vars and a prompt
4. **Agent work** — The agent calls Team Dashboard's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Team Dashboard and agent runtimes. Each adapter is a package with three modules:
...
- **Control plane, not execution plane** — Team Dashboard orchestrates agents; it doesn't run them
