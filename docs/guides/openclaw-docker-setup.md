...
Team Dashboard includes an end-to-end join smoke harness:
...
- probes and prints a Team Dashboard host URL that is reachable from inside OpenClaw Docker
...
If your Team Dashboard deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Team Dashboard server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Team Dashboard URL (typically `http://host.docker.internal:3100`).
- If Team Dashboard rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart Team Dashboard and rerun the smoke script.
