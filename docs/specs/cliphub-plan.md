# ClipHub: Marketplace for Team Dashboard Team Configurations

> **Cluster:** Specs · **Tags:** cliphub, marketplace, team-blueprint, company-package, agents · **Related:** [Companies Spec](../companies/companies-spec.md), [Agent Config UI](agent-config-ui.md), [Docs Index](../README.md)
...
> The "app store" for whole-company AI teams — pre-built Team Dashboard configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Team Dashboard-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Team Dashboard company (agents, projects, governance) |
...
A complete Team Dashboard company configuration:
...
Single-agent configurations designed to plug into a Team Dashboard org:
...
Portable skill files that any Team Dashboard agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Team Dashboard's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Team Dashboard version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Team Dashboard company
...
| `POST` | `/api/listings/:id/install` | Install to Team Dashboard company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Team Dashboard company (or create new)
  → ClipHub API calls Team Dashboard API to:
  ...
  → Redirect to Team Dashboard dashboard with new team running
```
...
### 5.3 Creator: Export from Team Dashboard → Publish

```
Running Team Dashboard company → "Export as Blueprint" (CLI or UI)
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Team Dashboard brand blue/purple)
...
| Install flow | One-click deploy to Team Dashboard company |
...
{
  "targetCompanyId": "uuid",         // Existing Team Dashboard company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Team Dashboard
- **Backend**: Node.js API (or extend Team Dashboard server)
- **Database**: Postgres (can share Team Dashboard's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Team Dashboard auth (or OAuth2)

### 9.2 Integration with Team Dashboard

ClipHub can be:
- **Option A**: A separate app that calls Team Dashboard's API to install blueprints
- **Option B**: A built-in section of the Team Dashboard UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Team Dashboard UI and API.
...
- [ ] Install flow: blueprint → Team Dashboard company
...
- [ ] "Export from Team Dashboard" CLI command
