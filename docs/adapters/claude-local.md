...
The adapter creates a temporary directory with symlinks to Team Dashboard skills and passes it via `--add-dir`. This makes skills discoverable without polluting the agent's working directory.
...
This installs Team Dashboard skills in `~/.claude/skills`, creates an agent API key, and prints shell exports to run as that agent.
