...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and Team Dashboard just triggers it.
...
1. Team Dashboard sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the Team Dashboard API
...
The external agent uses `PAPERCLIP_API_URL` and an API key to call back to Team Dashboard.
