...
Add first-class **documents** to Team Dashboard as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Team Dashboard workflow/docs.
...
Update the Team Dashboard agent workflow so planning no longer edits the issue description.
