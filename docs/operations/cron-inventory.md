...
- **CreditScore Content Drafts (1 job)**: On the 1st of each month at 10 AM UTC, generates AEO-optimized page drafts for every active Growth (2 pages) and Pro (4 pages) subscription via Claude Sonnet 4.6, targeting the weakest audit signals. Drafts land in `creditscore_content_drafts` with `status=pending_review` for board approval. Owner: `cipher`. Monthly (`creditscore:content-drafts`).
