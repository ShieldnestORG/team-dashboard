...
- **Daily AI Brief (1 job)**: `socials:daily-brief` — daily 07:15 (after Zernio Analytics), reads the last 7 days across Zernio snapshots/posts, `x_tweet_analytics` (own section, never blended with Zernio), `social_leads`, University email events, and every `new` `inspiration_items` row, then makes ONE `callLlmChat` call to write a plain-English brief into `daily_briefs` (upsert on `company_id`+`brief_date`). Defensive JSON parsing never crashes the cron — a bad LLM response stores a `fallback.rawText` section instead. Marks reviewed inspiration items with an AI comment. Owner `system`. See [docs/products/daily-brief.md](../products/daily-brief.md).
