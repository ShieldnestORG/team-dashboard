...
const visualKeys = {
  ...
  backends: ["visual", "backends"] as const,
};
  ...
  return (
    <Card className="rounded-xl">
      <CardContent className="space-y-3 pt-0">
        {/* Badges row */}
        <div className="flex items-center gap-2 flex-wrap">
          <Badge className={`text-xs ${visualPlatformColor(item.platform)}`}>
            {visualPlatformLabel(item.platform)}
          </Badge>
          <Badge className={`text-xs ${agentColor(item.agentId)}`}>
            {item.agentId}
          </Badge>
          <Badge className={`text-xs ${statusColor(item.status)}`}>
            {item.status}
          ...
          {item.reviewStatus && item.reviewStatus !== "pending" && (
            <Badge className={`text-xs ${reviewStatusColor(item.reviewStatus)}`}>
              {item.reviewStatus}
            </Badge>
          )}
          <Badge className="text-xs bg-muted/15 text-muted-foreground border-border">
            {item.backend}
          </Badge>
          <span className="ml-auto flex items-center gap-1 text-xs text-muted-foreground">
            <Clock className="h-3 w-3" />
        ...
        {item.status === "failed" && (
          <div className="flex items-center justify-center h-32 rounded-lg border border-red-500/20 bg-red-500/5">
            <div className="flex items-center gap-2">
        ...
        {item.reviewComment && (
          <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
            <p className="text-xs text-muted-foreground">
              <span className="font-medium">Review note:</span> {item.reviewComment}
            </p>
          </div>
        )}
...
function VisualContentSection() {
  ...
  const { data: queueData, isLoading } = useQuery({
    queryKey: visualKeys.queue(queryParams),
    queryFn: () => visualContentApi.listQueue(queryParams),
    refetchInterval: 15_000, // Poll more frequently for visual (jobs in progress)
  });

  const { data: stats } = useQuery({
    queryKey: visualKeys.stats,
    queryFn: () => visualContentApi.stats(),
  ...
  const reviewMutation = useMutation({
    ...
    }) => visualContentApi.reviewItem(id, reviewStatus, reviewComment),
        ...
        {VISUAL_PLATFORM_TABS.map((tab) => (
          <TabsContent key={tab.value} value={tab.value}>
            {isLoading ? (
              ...
              <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
                {items.map((item: VisualContentItem) => (
                  <VisualContentCard
                    key={item.id}
                    item={item}
