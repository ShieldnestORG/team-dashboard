...
// Canonical attribution vocabulary — these are the only values the backend
// (PUT /leads/:id/attribution) accepts, and they drive payout amounts via
// rateForAttribution() in directory-listings.ts. Keep this in lockstep with
// the DB default in packages/db/src/schema/referral_attribution.ts.
...
export const ATTRIBUTION_TYPE_LABEL: Record<string, string> = {
  affiliate_referred_cd_closed: "Referred",
  affiliate_assisted_cd_closed: "Assisted",
  affiliate_led_cd_finalized: "Led",
  cd_direct: "CD Direct",
  admin_override: "Override",
};

export const ATTRIBUTION_TYPE_BADGE: Record<string, string> = {
  // Standard-rate stories share the warm brand palette.
  affiliate_referred_cd_closed: "bg-[#ff876d]/15 text-[#ff876d] border-[#ff876d]/30",
  affiliate_assisted_cd_closed: "bg-blue-500/15 text-blue-600 border-blue-500/30",
  // Led pays a 25% bonus — give it a distinct, celebratory green.
  affiliate_led_cd_finalized: "bg-emerald-500/15 text-emerald-600 border-emerald-500/30",
  // CD-direct pays no affiliate commission — neutral/muted.
  cd_direct: "bg-slate-500/15 text-slate-600 border-slate-500/30",
  // Manual admin action — amber to flag it as hand-set.
  admin_override: "bg-amber-500/15 text-amber-600 border-amber-500/30",
};
