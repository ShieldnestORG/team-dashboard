...
- [Watchtower](docs/products/watchtower.md) — $49/mo brand-mention monitor across five AI answer engines (ChatGPT, Claude, Perplexity, Gemini, Grok). Weekly digest + read-only run viewer; admin at `/watchtower`. In-house alternative to Profound/Peec.
- [Daily AI Brief + Inspiration Board](docs/products/daily-brief.md) — Daily 07:15 cron reads the last 7 days across Zernio/X/leads/University email/Watchtower and writes a plain-English brief at `/daily-brief`; the team pastes saved-post links at `/inspiration` for the same run to review.
- [Content Hub — Deploy Notes](docs/deploy/content-hub-notes.md) — VPS4 env vars for the marketing Content Hub (`ELEVENLABS_VOICE_KEY` minting steps) + authenticated-mode E2E click-through results.
- [Eagan Desktop Setup](docs/eagan-desktop-setup.md) — Give-to-Eagan doc: install the marketing MCPB extension in Claude Desktop + the custom-instructions block (team guidelines, escalation-to-Mark clause, owner content-guidelines placeholder).
- [Eagan Access Runbook](docs/eagan-access-runbook.md) — Owner ops for the external-marketing board key: 90-day mint (two-step bootstrap — approval binds the key to the approver), revoke/rotate, audit-log queries, .mcpb build.
- [Marketing Content Hub Handoff](docs/marketing-content-hub-handoff.md) — Self-contained handoff for the Content Hub build: funnel-kit surface with live Zernio status, `/CD` path fixes, Tokns project switcher, ElevenLabs voice-snippet factory.
