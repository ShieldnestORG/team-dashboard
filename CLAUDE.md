...
- **Chain Metrics** — direct Cosmos SDK LCD ingestion (`server/src/services/cosmos-lcd.ts`) for staking APR, validator count, block height, inflation, community tax across cosmos/osmosis (and tx-blockchain when `TX_LCD_URL` is set). Computes APR locally via the canonical Cosmos formula `(annual_provisions × (1 - community_tax)) / bonded_tokens` — no vendor API key required. Mintscan was deprecated 2026-04-14 after a week of silent 401s; we now own the entire fetch path
- **Chain TVL** — DefiLlama free public API ingestion (`server/src/services/defillama.ts`) for cosmos/osmosis with 24h delta tracking, written as `report_type='chain-tvl'` rows
- **Validator Rank Tracking** — Firecrawl-scraped public validator pages (`server/src/services/firecrawl-validators.ts`) parsed into ranked validator tables. Top-50 rows + tracked moniker (e.g. ShieldNest) shadowed into `validator_rank_history` for time-series rank deltas. Powers the TX validator pitch ("ShieldNest moved up 2 ranks")
...
- **Intel Dashboard** — admin UI page at `/intel` with tabbed tables (Overview/Listings/Crypto/AI-ML/DeFi/DevTools), searchable company lists, stats cards
- **Directory Listings (monetization)** — Stripe-backed recurring subscriptions selling **featured/verified/boosted** placement on `directory.coherencedaddy.com` to the 532 indexed companies. Full sales-pipeline admin UI at `/intel` → **Listings** tab: smart search (name/slug/category/email/website/twitter), status chips (prospect / contacted / checkout_sent / active / past_due / canceled), stats cards (active listings, MRR, past due, prospects with email, total), and per-company detail drawer with editable contact info, Stripe Checkout link creation, cancel subscription, freeform notes, and event timeline. **Traffic attribution panel** shows content mentions + clicks driven for each company (joins `content_items` by name/slug) so we can prove value to prospects. Backend: `directory-listings` service + `/api/directory-listings/*` admin routes + `/api/stripe/webhook` (signature-verified via `req.rawBody`, filtered on `metadata.source=directory_listings`). Tiers: Featured $199/mo, Verified $499/mo, Boosted $1499/mo (Stripe price IDs via env). Schema: `directory_listings` + `directory_listing_events` audit log + 5 new `intel_companies.contact_*` columns. Public `GET /intel/companies` endpoint now includes `featured` + `tier` fields so directory.coherencedaddy.com can sort featured first
...
| `server/src/services/firecrawl-validators.ts` | Firecrawl-scraped validator-list pages → `validator_rank_history` |
...
| `COSMOS_LCD_URL` | Optional | VPS | Public Cosmos Hub LCD endpoint (default: `https://cosmos-rest.publicnode.com`) |
| `OSMOSIS_LCD_URL` | Optional | VPS | Public Osmosis LCD endpoint (default: `https://osmosis-rest.publicnode.com`) |
| `TX_LCD_URL` | Optional | VPS | TX Blockchain LCD endpoint — when set, `intel:chain-metrics` includes tx-blockchain |
| `TX_VALIDATORS_URL` | Optional | VPS | Public validator-list URL for TX chain — scraped by `intel:firecrawl-validators` |
