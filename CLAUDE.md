...
- **Validator Rank Tracking** — LCD-fetched validator sets (`server/src/services/cosmos-lcd.ts` → `ingestValidatorRanks()`) parsed into ranked tables. Top-50 rows + tracked moniker (e.g. ShieldNest) shadowed into `validator_rank_history` for time-series rank deltas. Powers the TX validator pitch ("ShieldNest moved up 2 ranks"). Cron: `intel:validator-ranks` every 6h
...
| `TX_VALIDATORS_URL` | Optional | VPS | Public validator-list URL for TX chain (reserved for future use; LCD covers cosmos/osmosis natively) |
