...
export function creditscoreRoutes(db: Db): Router {
  ...
  router.get("/competitor-scans", async (req: Request, res: Response) => {
    ...
    try {
      const scans = await competitorAgent.listRecent(50);
      res.json({ scans });
    } catch (err) {
      logger.error({ err }, "creditscore: listRecent competitor scans failed");
  ...
  router.get("/strategy-docs", async (req: Request, res: Response) => {
    ...
    try {
      const docs = await sageStrategist.listRecent(50);
      res.json({ docs });
    } catch (err) {
      logger.error({ err }, "creditscore: listRecent strategy docs failed");
