...
describe("privateHostnameGuard", () => {
  it("allows requests when disabled", async () => {
    ...
    const res = await request(local.via(app)).get("/api/health").set("Host", "dotta-macbook-pro:3100");
    expect(res.status).toBe(200);
  });

  it("allows loopback hostnames", async () => {
    const app = createApp({ enabled: true });
    const res = await request(local.via(app)).get("/api/health").set("Host", "localhost:3100");
    expect(res.status).toBe(200);
  });
  ...
  it("blocks unknown hostnames on page routes with plain-text remediation command", async () => {
    const app = createApp({ enabled: true, allowedHostnames: ["some-other-host"] });
    const res = await request(local.via(app)).get("/dashboard").set("Host", "dotta-macbook-pro:3100");
    expect(res.status).toBe(403);
