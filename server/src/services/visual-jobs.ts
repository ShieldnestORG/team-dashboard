...
export interface VisualJob {
  ...
  prompt: string;
  error?: string;
  /** Stored asset key (set when job completes and asset is persisted) */
...
// ---------------------------------------------------------------------------
// In-memory index for fast lookups during polling
// ---------------------------------------------------------------------------
...
/** Get all jobs that are still in-flight (need polling) */
...
// ---------------------------------------------------------------------------
// Polling loop — call this on an interval to check async jobs
// ---------------------------------------------------------------------------
...
export async function pollPendingJobs(): Promise<void> {
  ...
  for (const job of pending) {
    ...
    try {
      ...
      // Otherwise still generating — no update needed
    } catch (err) {
