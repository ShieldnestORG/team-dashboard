...
export async function spentTodayUsd(db: Db): Promise<number> {
  try {
    ...
    const rows = await db
      .select({ cost: universityAgentUsage.costUsd })
      .from(universityAgentUsage);
    // Filtering/summing in SQL is better at scale; kept simple here and bounded
    // by the daily row count. TODO(perf): push the date filter + SUM into SQL
    // once volume warrants (createdAt >= startOfDay).
    void startOfDay;
