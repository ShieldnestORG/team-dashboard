...
// ---------------------------------------------------------------------------
// Gemini visual generation backend
// Uses Google Generative AI — Imagen 3 for images, Veo 2 for video
// ---------------------------------------------------------------------------

const GEMINI_API_KEY = process.env.GEMINI_API_KEY || "";
const GEMINI_BASE_URL = "https://generativelanguage.googleapis.com/v1beta";

/** In-flight jobs that Gemini is processing (video is async) */
...
// ---------------------------------------------------------------------------
// Video generation via Veo 2 (async operation)
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    if (!prediction?.bytesBase64Encoded) {
      ...
      return {
        jobId,
        status: "failed",
        error: "Operation completed but no video data",
      };
    }
    ...
    return { jobId, status: "generating" }; // Treat transient errors as still generating
  }
}
